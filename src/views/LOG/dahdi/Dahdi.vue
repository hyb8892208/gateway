    ...
    export default {
        ...
        methods: {
            ...
            change_rate(){
                clearTimeout(this.timeoutID)
                this.timeoutID = setTimeout(this.get_log_content, this.refresh_rate*1000)
                this.setCookie("dahdi_cookies_val", this.refresh_rate)
            },
            refresh(){
                this.$message({
                    message: '重新加载成功',
            ...
            clean_up(){
                this.$confirm('确定要清空Dahdi日志吗？')
                    .then(_ => {
                        this.request.AGLogDelAll(this.clean_succeed_back, this.clean_error_back, 'log-dahdi')
                    })
                    .catch(_ => {
