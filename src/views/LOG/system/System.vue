    ...
    export default {
        ...
        methods: {
            ...
            change_rate(){
                clearTimeout(this.timeoutID)
                this.timeoutID = setTimeout(this.get_log_content, this.refresh_rate*1000)
                this.setCookie("system_cookies_val", this.refresh_rate)
            },
            refresh(){
                this.$message({
                    message: '重新加载成功',
            ...
            clean_up(){
                this.$confirm('确定要清空System日志吗？')
                    .then(_ => {
                        this.request.AGLogDelAll(this.clean_succeed_back, this.clean_error_back, 'log-sys')
                    })
                    .catch(_ => {
        ...
        created() {
            ...
            let cookies_val = this.getCookie('system_cookies_val')
            if(cookies_val == null){
                this.setCookie("system_cookies_val", this.refresh_rate)
            }else{
